...
#if 1
        double s1 = - sqrt(-2*(-alimit)*( xlimit - xx))/(-alimit*dt);
        double s2 =   sqrt(-2*( alimit)*(-xlimit - xx))/( alimit*dt);
        double vmod;
        vmod = (( xlimit - xx) - 0.5*(s1*s1 - s1)*(-alimit)*dt*dt)/((s1+1)*dt);
        //if (v < vmax) vmax = vmod;
        vmax = vmod;
        vmod = - ((-xlimit - xx) - 0.5*(s2*s2 - s2)*( alimit)*dt*dt)/((s2+1)*dt);
        //if (v < vmin) vmin = vmod;
        vmin = vmod;
#endif
