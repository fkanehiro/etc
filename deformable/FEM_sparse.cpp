...
void FEM::assembleK()
{
	...
	{		
        ...
        std::cout << "non zero of K:" << mK_.nonZeros() << "/(" 
                  << mK_.cols() << "x" << mK_.rows() << "), (" 
                  << ((double)mK_.nonZeros())/(mK_.cols()*mK_.rows())*100 
                  << "%)"  << std::endl;
}

void FEM::sortK()
{
	DynamicSparseMatrix<double> sK(3*(fNodeSize_+iNodeSize_),3*(fNodeSize_+iNodeSize_));
	sK.setZero();
        std::cout << "after sK is allocated(hit return key)" << std::flush; getchar();
	...
	for (int i=0 ; i < iNodeSize_ ; ++i){
        ...
        std::cout << "before sorting(hit return key)" << std::flush; getchar();
	//Computation of the organized Matrices
        SparseMatrix<double> m(mK_);
        for (int k=0; k<m.outerSize(); ++k)
            for (SparseMatrix<double>::InnerIterator it(m,k); it; ++it)
                {
                    sK.coeffRef(sortmap(it.row()), sortmap(it.col())) = it.value();
                }        
	// copy the Stiffness matrix
        std::cout << "non zero of sK:" << sK.nonZeros() << "/(" 
                  << sK.cols() << "x" << sK.rows() << "), (" 
                  << ((double)sK.nonZeros())/(sK.cols()*sK.rows())*100 
                  << "%)"  << std::endl;
	mK_ = sK;
...
{
#if 0
	cK_.resize(3*fNodeSize_,3*fNodeSize_);
	if(cK_.size() > 0){
		// Compute the condensed stiffness matrix
		cK_ = mK_.block(0,0,3*fNodeSize_,3*fNodeSize_)
			- mK_.block(0,3*fNodeSize_,3*fNodeSize_,3*iNodeSize_)
			* (MatrixXd)(mK_.block(3*fNodeSize_,3*fNodeSize_, 3*iNodeSize_,3*iNodeSize_)).inverse()
			* mK_.block(3*fNodeSize_,0, 3*iNodeSize_,3*fNodeSize_);
		
#if 1 
		std::cout << "mK:" << mK_ << std::endl;
		std::cout << "cK:" << cK_ << std::endl;
#endif
		cK_.computeInverse(&cK_);
#if 1 
		std::cout << "inverse of cK:" << cK_ << std::endl;
#endif
		// for debug purpose, to be removed
...
#else
        std::cout << "don't call computeInverseForceCondensedK()" << std::endl;
#endif
}
...
void FEM::computeDisplacementCondensedK()
{
	...
	if(dK_.size() > 0){
		...
		dK_ = -mKDtmp.block(0,0,3*fNodeSize_,3*dNodeSize_)
			   +mK_.block(0,3*fNodeSize_,3*fNodeSize_,3*iNodeSize_)
			   *(MatrixXd)(mK_.block(3*fNodeSize_,3*fNodeSize_, 3*iNodeSize_,3*iNodeSize_)).inverse()
			   *mKDtmp.block(3*fNodeSize_,0, 3*iNodeSize_,3*dNodeSize_);
	...
	cout << "------------------------------------------------------" << endl << endl;
#else
        std::cout << "don't call computeDisplacementCondensedK()" << std::endl;
#endif
}	
...
const MatrixXd & FEM::getcK() const{
    std::cout << "don't call FEM::getcK()" << std::endl;
	return cK_;
}
#if 1
...
#endif
const DynamicSparseMatrix<double>& FEM::getsK() const{
    return mK_;
}

const MatrixXd& FEM::getKD() const{
    return mKD_;
}

const MatrixXd & FEM::getdK() const{
    std::cout << "don't call FEM::getdK()" << std::endl;
	return dK_;
}
