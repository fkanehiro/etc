...
int main(int argc, char *argv[])
{
    ...
    std::cout << "assembleK():" << t2.tv_sec - t1.tv_sec << std::endl;
    t1 = t2;
    stopper_->sortK();
    gettimeofday(&t2, NULL);
    std::cout << "sortK():" << t2.tv_sec - t1.tv_sec << std::endl;
    t1 = t2;
    stopper_->sortT();
    gettimeofday(&t2, NULL);
    std::cout << "sortT():" << t2.tv_sec - t1.tv_sec << std::endl;
    t1 = t2;
#if 1
    stopper_->computeInverseForceCondensedK();
    //saveMatrix("04_stopper_rotation_cK.mat", stopper_->getcK());
#else
    MatrixXd m;
    loadMatrix("04_stopper_rotation_cK.mat", m);
    stopper_->setcK(m);
#endif
    gettimeofday(&t2, NULL);
    std::cout << "computeInverseForceCondensedK():" << t2.tv_sec - t1.tv_sec << std::endl;
    t1 = t2;
    stopper_->computeDisplacementCondensedK();
    gettimeofday(&t2, NULL);
    std::cout << "computeDisplacementCondensedK():" << t2.tv_sec - t1.tv_sec << std::endl;
    t1 = t2;
