...
static const double DEFAULT_NEGATIVE_VELOCITY_RATIO_FOR_PENETRATION = 0.0;
...
namespace hrp
{
    ...
    public:
        ...
        void solve(const std::vector<CollisionDataSequence>& i_cd);
...
#define PI_2 M_PI/2
        ...
        */
        ...
        void setConstraintPoints(const std::vector<CollisionDataSequence>& collisions);
        void setContactConstraintPoints(LinkPair& linkPair, const CollisionDataSequence& collisionPoints);
...
void CFSImpl::solve(const std::vector<CollisionDataSequence>& i_cd)
{
    if(CFS_DEBUG)
    ...
    if(globalNumConstraintVectors > 0){
...
#if 1
        //std::cout << "globalNumConstraintVectors:" << globalNumConstraintVectors << std::endl;
        //std::cout << "globalNumFrictionVectors:" << globalNumFrictionVectors << std::endl;
        //std::cout << "A:" << Mlcp << std::endl;
        //std::cout << "a0+v0/dt:" << b << std::endl;
#if 1
        double dtdt = world.timeStep()*world.timeStep();
        Mlcp *= dtdt;
        //Mlcp = dzeromatrix(Mlcp.size1(), Mlcp.size2());
        b *= dtdt;
        // TODO : generalize
        for(size_t colIndex=0; colIndex < collisionCheckLinkPairs.size(); ++colIndex){
            ...
            for (int i=0; i<2; i++){
                DeformableLink *dl;
                dl = dynamic_cast<DeformableLink *>(linkPair.link[i]);
                if (dl){
                    dmatrix HCHt = dl->computeW(i_cd[colIndex]);
                    if (HCHt.size1() != (int)Mlcp.size1()){
                        std::cout << "HCHt:" << HCHt.size1() << " x " << HCHt.size2() 
                                  << ", Mlcp:" << Mlcp.size1() << " x " << Mlcp.size2()
                                  << std::endl;
                        exit(0);
                    }
                    //std::cout << "HCHt:" << HCHt << std::endl;
                    Mlcp += HCHt;
                    b += dl->getDeltaFree();
                    //b = delta_free;
                }
        ...
        //getchar();
        // update shape of deformable object
        for(size_t colIndex=0; colIndex < collisionCheckLinkPairs.size(); ++colIndex){
            LinkPair& linkPair = *collisionCheckLinkPairs[colIndex];
            for (int i=0; i<2; i++){
                DeformableLink *dl;
                ...
                if (dl){
                    // TODO : generalize
                    if (solution.size() > 0){
                        dvector localF = prod(dl->getHt(), solution);
                        VectorXd eF (dl->getSizeF());
                        eF.setZero();
                        VectorXi& indexDefNode = dl->getIndexDefNode();
                        for (unsigned int i=0; i<solution.size(); i++){
                            int rank = indexDefNode(i_cd[0][i].id1);
                            eF(rank*3+0) = localF[i*3];
                            eF(rank*3+1) = localF[i*3+1];
                            eF(rank*3+2) = localF[i*3+2];
                        }
                        dl->setForce(eF);
                        dl->solveU();
                        dl->updateNodes();
...
void CFSImpl::setConstraintPoints(const std::vector<CollisionDataSequence>& i_cd)
{
    const CollisionDataSequence* pCollisionPoints = 0;

    for(size_t colIndex=0; colIndex < collisionCheckLinkPairs.size(); ++colIndex){
        pCollisionPoints = 0;
        LinkPair& linkPair = *collisionCheckLinkPairs[colIndex];

        const CollisionDataSequence& points = i_cd[colIndex];
        if(points.size() > 0){
...
void CFSImpl::setContactConstraintPoints(LinkPair& linkPair, const CollisionDataSequence& collisionPoints)
{
    ...
    for(int j=0; j < numContactsInPair; ++j){
        ...
        const CollisionData& collision = collisionPoints[j];
...
#if 0
                    std::cout << "depth:" << constraint.depth << std::endl;
                    std::cout << "normal:" << constraint.normalTowardInside[0] << std::endl;
                    std::cout << "point:" << constraint.point << std::endl;
#endif
...
ConstraintForceSolverExt::ConstraintForceSolverExt(WorldBase& world)
{
    impl = new CFSImpl(world);
}


ConstraintForceSolverExt::~ConstraintForceSolverExt()
{
    delete impl;
}


bool ConstraintForceSolverExt::addCollisionCheckLinkPair
(int bodyIndex1, Link* link1, int bodyIndex2, Link* link2, double muStatic, double muDynamic, double culling_thresh, double epsilon)
{
...
void ConstraintForceSolverExt::clearCollisionCheckLinkPairs()
{
...
void ConstraintForceSolverExt::setGaussSeidelParameters(int maxNumIteration, int numInitialIteration, double maxRelError)
{
...
void ConstraintForceSolverExt::enableConstraintForceOutput(bool on)
{
    impl->isConstraintForceOutputMode = on;
}


void ConstraintForceSolverExt::setNegativeVelocityRatioForPenetration(double ratio)
{
...
void ConstraintForceSolverExt::initialize(void)
{
    impl->initialize();
}


void ConstraintForceSolverExt::solve(const std::vector<CollisionDataSequence>& i_cd)
{
    impl->solve(i_cd);
}


void ConstraintForceSolverExt::clearExternalForces()
{
    impl->clearExternalForces();
}

void ConstraintForceSolverExt::setAllowedPenetrationDepth(double dVal)
{
    impl->allowedPenetrationDepth = dVal;
}

double ConstraintForceSolverExt::getAllowedPenetrationDepth() const
{
