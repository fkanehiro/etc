...
public:
  ...
  region(double cx, double cy, double cz,
  ...
  bool contain(nodePtr n){
    ...
    return (x >= m_cx - m_wx/2 && x <= m_cx + m_wx/2
	    ...
	    && z >= m_cz - m_wz/2 && z <= m_cz + m_wz/2);
  }
private:
  double m_cx, m_cy, m_cz, m_wx, m_wy, m_wz;
};
...
public:
  ...
  nodePtr sample(){
    return nodePtr(new node((m_searchRegion.wx()*rand())/RAND_MAX
			    ...
			    (m_searchRegion.wz()*rand())/RAND_MAX
			    + m_searchRegion.cz()-m_searchRegion.wz()/2));
  }
