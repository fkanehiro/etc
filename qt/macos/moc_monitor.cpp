/****************************************************************************
** Meta object code from reading C++ file 'monitor.h'
**
** Created: Sun Apr 15 21:11:43 2012
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../monitor.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'monitor.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_monitor[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       7,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       9,    8,    8,    8, 0x08,
      22,   16,    8,    8, 0x08,
      36,    8,    8,    8, 0x08,
      45,    8,    8,    8, 0x08,
      54,    8,    8,    8, 0x08,
      62,    8,    8,    8, 0x08,
      74,   69,    8,    8, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_monitor[] = {
    "monitor\0\0play()\0value\0playJust(int)\0"
    "faster()\0slower()\0start()\0stop()\0nLog\0"
    "intervalSimulation(uint)\0"
};

const QMetaObject monitor::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_monitor,
      qt_meta_data_monitor, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &monitor::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *monitor::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *monitor::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_monitor))
        return static_cast<void*>(const_cast< monitor*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int monitor::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: play(); break;
        case 1: playJust((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: faster(); break;
        case 3: slower(); break;
        case 4: start(); break;
        case 5: stop(); break;
        case 6: intervalSimulation((*reinterpret_cast< uint(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 7;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
